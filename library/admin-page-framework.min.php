...
class AdminPageFramework_Debug extends AdminPageFramework_WPUtility {
    ...
    static private function _getLogHeadingLine($fCurrentTimeStamp, $nElapsed, $sCallerClass, $sCallerFunction) {
        ...
        $_aOutput = array(date("Y/m/d H:i:s", $_nNow) . '.' . $_nMicroseconds, self::_getFormattedElapsedTime($nElapsed), $_iPageLoadID, AdminPageFramework_Registry::VERSION . (AdminPageFramework_Registry::$bIsMinifiedVersion ? '.min' : ''), $sCallerClass . '::' . $sCallerFunction, current_filter(), self::getCurrentURL(),);
        return implode(' ', $_aOutput);
    }
...
class AdminPageFramework_FieldType_image extends AdminPageFramework_FieldType {
    ...
    protected function _getRemoveButtonScript($sInputID, array $aButtonAttributes, $sType = 'image') {
        ...
        $_sScript = "if(0===jQuery('a#remove_{$sType}_{$sInputID}').length)jQuery('input#{$sInputID}').after($_sButtonHTML);";
