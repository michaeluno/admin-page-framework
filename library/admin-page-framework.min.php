<?php 
/**
	Admin Page Framework - Minified Version v3.5.3b19 by Michael Uno 
...
abstract class AdminPageFramework_Factory_Controller extends AdminPageFramework_Factory_View {
    ...
    public function setFieldErrors($aErrors) {
        ...
        $_sID = md5($this->oProp->sClassName);
        $GLOBALS['aAdminPageFramework']['aFieldErrors'][$_sID] = $this->oUtil->getElement($GLOBALS, array('aAdminPageFramework', 'aFieldErrors', $_sID), $aErrors);
    }
...
abstract class AdminPageFramework_UserMeta_Model extends AdminPageFramework_UserMeta_Router {
    ...
    private function _updatePostMeta($iUserID, array $aInput, array $aSavedMeta) {
        ...
        foreach ($aInput as $_sSectionOrFieldID => $_vValue) {
            ...
            $_vSavedValue = $this->oUtil->getElement($aSavedMeta, $_sSectionOrFieldID, null);
            if ($_vValue == $_vSavedValue) {
                continue;
            }
    ...
    protected function _getInputArray(array $aFieldDefinitionArrays, array $aSectionDefinitionArrays) {
        ...
        foreach ($aFieldDefinitionArrays as $_sSectionID => $_aSubSectionsOrFields) {
            if ('_default' == $_sSectionID) {
                $_aFields = $_aSubSectionsOrFields;
                foreach ($_aFields as $_aField) {
                    $_aInput[$_aField['field_id']] = $this->oUtil->getElement($_POST, $_aField['field_id'], null);
                }
                continue;
            }
            $_aInput[$_sSectionID] = $this->oUtil->getElementAsArray($_aInput, $_sSectionID, array());
            if (!count($this->oUtil->getIntegerKeyElements($_aSubSectionsOrFields))) {
                $_aFields = $_aSubSectionsOrFields;
                foreach ($_aFields as $_aField) {
                    $_aInput[$_sSectionID][$_aField['field_id']] = $this->oUtil->getElement($_POST, array($_sSectionID, $_aField['field_id']), null);
                }
                continue;
            }
            foreach ($_POST[$_sSectionID] as $_iIndex => $_aFields) {
                $_aInput[$_sSectionID][$_iIndex] = $this->oUtil->getElement($_POST, array($_sSectionID, $_iIndex), null);
            }
        }
        return $_aInput;
    }
...
class AdminPageFramework_FormElement extends AdminPageFramework_FormElement_Utility {
    ...
    public function formatFields(array $aFields, $sFieldsType, $sCapability) {
        ...
        foreach ($aFields as $_sSectionID => $_aSubSectionsOrFields) {
            ...
            $_aNewFields[$_sSectionID] = $this->getElement($_aNewFields, $_sSectionID, array());
    ...
    public function setDynamicElements($aOptions) {
        ...
        foreach ($aOptions as $_sSectionID => $_aSubSectionOrFields) {
            ...
            foreach ($_aSubSectionOrFields as $_isIndexOrFieldID => $_aSubSectionOrFieldOptions) {
                ...
                $_aSubSection[$_iIndex] = isset($this->aConditionedFields[$_sSectionID][$_iIndex]) ? $this->aConditionedFields[$_sSectionID][$_iIndex] : $this->getNonIntegerKeyElements($this->aConditionedFields[$_sSectionID]);
...
class AdminPageFramework_FieldType_image extends AdminPageFramework_FieldType {
    ...
    protected function getExtraInputFields(&$aField) {
        ...
        foreach (( array )$aField['attributes_to_store'] as $sAttribute) {
            $_aOutputs[] = "<input " . $this->generateAttributes(array('id' => "{$aField['input_id']}_{$sAttribute}", 'type' => 'hidden', 'name' => "{$aField['_input_name']}[{$sAttribute}]", 'disabled' => isset($aField['attributes']['disabled']) && $aField['attributes']['disabled'] ? 'disabled' : null, 'value' => isset($aField['attributes']['value'][$sAttribute]) ? $aField['attributes']['value'][$sAttribute] : '',)) . "/>";
        }
...
class AdminPageFramework_FieldType_system extends AdminPageFramework_FieldType {
    ...
    private function _getSiteInfo() {
        global $wpdb;
        return array(__('Version', 'admin-page-framework') => $GLOBALS['wp_version'], __('Language', 'admin-page-framework') => $this->getSiteLanguage(), __('Memory Limit', 'admin-page-framework') => $this->getReadableBytes($this->getNumberOfReadableSize(WP_MEMORY_LIMIT)), __('Multi-site', 'admin-page-framework') => $this->getYesOrNo(is_multisite(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Permalink Structure', 'admin-page-framework') => get_option('permalink_structure'), __('Active Theme', 'admin-page-framework') => $this->_getActiveThemeName(), __('Registered Post Statuses', 'admin-page-framework') => implode(', ', get_post_stati()), 'WP_DEBUG' => $this->getYesOrNo($this->isDebugModeEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_LOG' => $this->getYesOrNo($this->isDebugLogEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_DISPLAY' => $this->getYesOrNo($this->isDebugDisplayEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Table Prefix', 'admin-page-framework') => $wpdb->prefix, __('Table Prefix Length', 'admin-page-framework') => strlen($wpdb->prefix), __('Table Prefix Status', 'admin-page-framework') => $this->getYesOrNo(strlen($wpdb->prefix) > 16, $this->oMsg->get('acceptable'), $this->oMsg->get('too_long')), 'wp_remote_post()' => $this->_getWPRemotePostStatus(), 'wp_remote_get()' => $this->_getWPRemoteGetStatus(), __('WP_CONTENT_DIR Writeable', 'admin-page-framework') => $this->getYesOrNo(is_writable(WP_CONTENT_DIR), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Active Plugins', 'admin-page-framework') => PHP_EOL . $this->_getActivePlugins(), __('Network Active Plugins', 'admin-page-framework') => PHP_EOL . $this->_getNetworkActivePlugins(), __('Constants', 'admin-page-framework') => $this->getDefinedConstants('user'),);
    }
    ...
    private function _getWPRemotePostStatus() {
        ...
        return $this->getYesOrNo(!is_wp_error($_vResponse) && $_vResponse['response']['code'] >= 200 && $_vResponse['response']['code'] < 300, $this->oMsg->get('functional'), $this->oMsg->get('not_functional'));
    }
    ...
    private function _getPHPInfo($bGenerateInfo = true) {
        ...
        self::$_aPHPInfo = array(__('Version', 'admin-page-framework') => phpversion(), __('Safe Mode', 'admin-page-framework') => $this->getYesOrNo(@ini_get('safe_mode'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Memory Limit', 'admin-page-framework') => @ini_get('memory_limit'), __('Upload Max Size', 'admin-page-framework') => @ini_get('upload_max_filesize'), __('Post Max Size', 'admin-page-framework') => @ini_get('post_max_size'), __('Upload Max File Size', 'admin-page-framework') => @ini_get('upload_max_filesize'), __('Max Execution Time', 'admin-page-framework') => @ini_get('max_execution_time'), __('Max Input Vars', 'admin-page-framework') => @ini_get('max_input_vars'), __('Argument Separator', 'admin-page-framework') => @ini_get('arg_separator.output'), __('Allow URL File Open', 'admin-page-framework') => $this->getYesOrNo(@ini_get('allow_url_fopen'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Display Errors', 'admin-page-framework') => $this->getYesOrNo(@ini_get('display_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Log Errors', 'admin-page-framework') => $this->getYesOrNo(@ini_get('log_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Error log location', 'admin-page-framework') => @ini_get('error_log'), __('Error Reporting Level', 'admin-page-framework') => $_oErrorReporting->getErrorLevel(), __('FSOCKOPEN', 'admin-page-framework') => $this->getYesOrNo(function_exists('fsockopen'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('cURL', 'admin-page-framework') => $this->getYesOrNo(function_exists('curl_init'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SOAP', 'admin-page-framework') => $this->getYesOrNo(class_exists('SoapClient'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SUHOSIN', 'admin-page-framework') => $this->getYesOrNo(extension_loaded('suhosin'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), 'ini_set()' => $this->getYesOrNo(function_exists('ini_set'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')),) + $this->getPHPInfo() + array(__('Constants', 'admin-page-framework') => $this->getDefinedConstants(null, 'user'));
        return self::$_aPHPInfo;
    }
    private function _getWebServerInfo($bGenerateInfo = true) {
        return $bGenerateInfo ? array(__('Web Server', 'admin-page-framework') => $_SERVER['SERVER_SOFTWARE'], 'SSL' => $this->getYesOrNo(is_ssl(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Session', 'admin-page-framework') => $this->getYesOrNo(isset($_SESSION), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Session Name', 'admin-page-framework') => esc_html(@ini_get('session.name')), __('Session Cookie Path', 'admin-page-framework') => esc_html(@ini_get('session.cookie_path')), __('Session Save Path', 'admin-page-framework') => esc_html(@ini_get('session.save_path')), __('Session Use Cookies', 'admin-page-framework') => $this->getYesOrNo(@ini_get('session.use_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Session Use Only Cookies', 'admin-page-framework') => $this->getYesOrNo(@ini_get('session.use_only_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')),) + $_SERVER : '';
    }
}
