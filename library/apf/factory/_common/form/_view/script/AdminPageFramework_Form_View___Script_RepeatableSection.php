...
JAVASCRIPTS;
    ...
    static public function getEnabler($sContainerTagID, $iSectionCount, $asArguments, $oMsg) {
        ...
        $_sButtons = "<div class='admin-page-framework-repeatable-section-buttons-outer-container'>" . "<div " . self::getAttributes(self::___getContainerAttributes($_aArguments, $oMsg)) . ' ' . self::getDataAttributes($_aArguments) . '>' . "<a " . self::getAttributes(self::___getRemoveButtonAttributes($sContainerTagID, $oMsg, $iSectionCount)) . ">-</a>" . "<a " . self::getAttributes(self::___getAddButtonAttributes($sContainerTagID, $oMsg, $_aArguments)) . ">+</a>" . "</div>" . "</div>" . self::___getModalForDisabledMessage($sContainerTagID, $_aArguments);
...
JAVASCRIPTS;
    ...
    static private function ___getArgumentsFormatted($asArguments, $oMsg) {
        ...
        unset($_aArguments[0]);
        $_aArguments = $_aArguments + array('min' => 0, 'max' => 0, 'disabled' => false,);
        if (!empty($_aArguments['disabled'])) {
            $_aArguments['disabled'] = $_aArguments['disabled'] + array('label' => $oMsg->get('repeatable_section_is_disabled'), 'caption' => $oMsg->get('warning_caption'), 'box_width' => 300, 'box_height' => 100,);
        }
        return $_aArguments;
    }
    static private function ___getModalForDisabledMessage($sContainerTagID, $aArguments) {
        ...
        add_thickbox();
        return "<div id='repeatable_section_disabled_{$sContainerTagID}' style='display:none'>" . "<p>" . $aArguments['disabled']['label'] . "</p>" . "</div>";
    }
    static private function ___getContainerAttributes(array $aArguments, $oMsg) {
        return array('class' => self::getClassAttribute('admin-page-framework-repeatable-section-buttons', empty($aArguments['disabled']) ? '' : 'disabled'),);
    }
    static private function ___getRemoveButtonAttributes($sContainerTagID, $oMsg, $iSectionCount) {
        return array('class' => 'repeatable-section-remove-button button-secondary ' . 'repeatable-section-button button button-large', 'title' => $oMsg->get('remove_section'), 'style' => $iSectionCount <= 1 ? 'display:none' : null, 'data-id' => $sContainerTagID,);
    }
    static private function ___getAddButtonAttributes($sContainerTagID, $oMsg, $aArguments) {
        return array('class' => 'repeatable-section-add-button button-secondary ' . 'repeatable-section-button button button-large', 'title' => $oMsg->get('add_section'), 'data-id' => $sContainerTagID, 'href' => !empty($aArguments['disabled']) ? '#TB_inline?width=' . $aArguments['disabled']['box_width'] . '&height=' . $aArguments['disabled']['box_height'] . '&inlineId=' . 'repeatable_section_disabled_' . $sContainerTagID : '',);
    }
}
