<?php 
/**
	Admin Page Framework v3.8.0b05 by Michael Uno 
...
class AdminPageFramework_Form_View___Fieldset extends AdminPageFramework_Form_View___Fieldset_Base {
    ...
    private function _getFieldsOutput(array $aFieldset, array $aFields, array $aCallbacks = array()) {
        if ($this->hasFieldDefinitionsInContent($aFieldset)) {
            return $this->_getChildFieldsets($aFieldset, $aFields);
        }
        $_aOutput = array();
    ...
    private function _getChildFieldsets(array $aParentFieldset, array $aFields) {
        $_aMethodNames = array(0 => '_getNestedFieldsetsBySubFieldIndex', 1 => '_getInlineMixedFieldsetsBySubFieldIndex',);
        $_sMethodName = $_aMethodNames[( integer )$this->hasInlineMixedFields($aParentFieldset) ];
        $_sNestedFieldsets = '';
        foreach ($aFields as $_isIndex => $_aField) {
            $_sNestedFieldsets.= $this->{$_sMethodName}($_isIndex, $_aField, $aParentFieldset, $this->isLastElement($aFields, $_isIndex), $this->hasSubFields($aFields, $_aField));
        }
    ...
    private function _getInlineMixedFieldsetsBySubFieldIndex($iIndex, $aField, array $aParentFieldset, $bIsLastElement = false, $bHasSubFields = false) {
        ...
        $aParentFieldset['type'] = '_mixed';
        $_oSubFieldFormatter = new AdminPageFramework_Form_Model___Format_EachField($aParentFieldset, $iIndex, $this->aCallbacks, $this->_getFieldTypeDefinition($aField['type']));
        ...
        foreach ($_aParentFieldset['content'] as $_aChildFieldset) {
            ...
            $_aChildFieldset = $this->_getFieldsetReformattedBySubFieldIndex($_aChildFieldset, ( integer )$iIndex, $bHasSubFields, $_aParentFieldset);
            $_oFieldset = new AdminPageFramework_Form_View___Fieldset($_aChildFieldset, $this->aOptions, array(), $this->aFieldTypeDefinitions, $this->oMsg, $this->aCallbacks);
            $_aInlineMixedOutput[] = $_oFieldset->get();
        }
        return $this->_getFieldOutput(call_user_func_array('sprintf', $_aInlineMixedOutput), $_aParentFieldset, $bIsLastElement);
    }
    private function _getNestedFieldsetsBySubFieldIndex($iIndex, array $aField, array $aParentFieldset, $bIsLastElement = false, $bHasSubFields = false) {
        $aParentFieldset['_is_multiple_fields'] = $bHasSubFields;
        $aParentFieldset['type'] = '_nested';
        $_oSubFieldFormatter = new AdminPageFramework_Form_Model___Format_EachField($aParentFieldset, $iIndex, $this->aCallbacks, $this->_getFieldTypeDefinition($aField['type']));
        $_aParentFieldset = $_oSubFieldFormatter->get();
        $_sNestedFieldsetsOutput = '';
        foreach ($_aParentFieldset['content'] as $_aNestedFieldset) {
            $_aNestedFieldset = $this->_getFieldsetReformattedBySubFieldIndex($_aNestedFieldset, ( integer )$iIndex, $bHasSubFields, $_aParentFieldset);
            $_oFieldset = new AdminPageFramework_Form_View___Fieldset($_aNestedFieldset, $this->aOptions, array(), $this->aFieldTypeDefinitions, $this->oMsg, $this->aCallbacks);
            $_sNestedFieldsetsOutput.= $_oFieldset->get();
        }
        return $this->_getFieldOutput($_sNestedFieldsetsOutput, $_aParentFieldset, $bIsLastElement);
    }
    private function _getFieldsetReformattedBySubFieldIndex(array $aFieldset, $iSubFieldIndex, $bHasSubFields, array $aParentFieldset) {
        $aFieldset['_parent_field_path'] = $this->getAOrB($bHasSubFields, $aFieldset['_parent_field_path'] . '|' . $iSubFieldIndex, $aFieldset['_parent_field_path']);
        $aFieldset['_parent_tag_id'] = $this->getAOrB($bHasSubFields, $aParentFieldset['tag_id'] . '__' . $iSubFieldIndex, $aParentFieldset['tag_id']);
        ...
        $_oFieldsetOutputFormatter = new AdminPageFramework_Form_Model___Format_FieldsetOutput($aFieldset, $aFieldset['_section_index'], $this->aFieldTypeDefinitions);
    ...
    private function _getFinalOutput(array $aFieldset, array $aFieldsOutput, $iFieldsCount) {
        ...
        return $aFieldset['before_fieldset'] . "<fieldset " . $_oFieldsetAttributes->get() . ">" . $this->_getChildFieldTitle($aFieldset) . $this->_getFieldsetContent($aFieldset, $aFieldsOutput, $iFieldsCount) . $this->_getExtras($aFieldset, $iFieldsCount) . "</fieldset>" . $aFieldset['after_fieldset'];
    }
    private function _getChildFieldTitle(array $aFieldset) {
        ...
        return "<label class='admin-page-framework-child-field-title' for=''>" . $aFieldset['title'] . "</label>";
    }
