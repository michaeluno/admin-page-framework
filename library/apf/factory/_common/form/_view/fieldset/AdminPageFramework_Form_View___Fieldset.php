<?php 
/**
	Admin Page Framework v3.8.0b04 by Michael Uno 
...
class AdminPageFramework_Form_View___Fieldset extends AdminPageFramework_Form_View___Fieldset_Base {
    ...
    private function _getFieldsOutput(array $aFieldset, array $aFields, array $aCallbacks = array()) {
        if ($this->hasNestedFields($aFieldset)) {
            return $this->_getNestedFieldsets($aFieldset, $aFields);
        }
        $_aOutput = array();
    ...
    private function _getNestedFieldsets(array $aParentFieldset, array $aFields) {
        $_sNestedFieldsets = '';
        foreach ($aFields as $_isIndex => $_aField) {
            $_sNestedFieldsets.= $this->_getNestedFieldsetsBySubFieldIndex($_isIndex, $_aField, $aParentFieldset, $this->isLastElement($aFields, $_isIndex), $this->hasSubFields($aFields, $_aField));
        }
    ...
    private function _getNestedFieldsetsBySubFieldIndex($iIndex, array $aField, array $aParentFieldset, $bIsLastElement = false, $bHasSubFields = false) {
        ...
        return $_aParentFieldset['before_field'] . "<div " . $_oFieldAttribute->get() . ">" . $_sNestedFieldsetsOutput . $this->_getUnsetFlagFieldInputTag($_aParentFieldset) . $this->_getDelimiter($_aParentFieldset, $bIsLastElement) . "</div>" . $_aParentFieldset['after_field'];
    }
    ...
    private function _getFinalOutput(array $aFieldset, array $aFieldsOutput, $iFieldsCount) {
        ...
        return $aFieldset['before_fieldset'] . "<fieldset " . $_oFieldsetAttributes->get() . ">" . $this->_getNestedFieldTitle($aFieldset) . $this->_getFieldsetContent($aFieldset, $aFieldsOutput, $iFieldsCount) . $this->_getExtras($aFieldset, $iFieldsCount) . "</fieldset>" . $aFieldset['after_fieldset'];
    }
    private function _getNestedFieldTitle(array $aFieldset) {
        ...
        return "<label class='admin-page-framework-nested-field-title' for=''>" . $aFieldset['title'] . "</label>";
    }
