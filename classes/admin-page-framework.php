	...
	*/
	...
	class Admin_Page_Framework {
		...
		protected $arrRootMenuSlugs = array(
			// all keys muse be lower case to support caese insensitive lookups.
			'dashboard' => 			'index.php',
			'posts' => 				'edit.php',
			'media' => 				'upload.php',
		...
		*/
		...
		/*
			Front-End methods - the user may call it but it shoud not nocessaliry be customized in the extended class.
		*/
		...
		*/
			...
			*/
			...
			foreach( ( array ) $arrFields as $index => $arrField ) {
				...
				if ( $arrField['type'] == 'image' ) {
					...
					// These two hooks should be enabled when the image field type was added in the field array.
		...
		function RenderFormField( $strMethodName, $arrField ) {
			...
			$arrErrors = get_transient( get_class( $this ) . '_' . $arrField['page_slug'] );
		...
		function RenderFormFieldByType( &$arrOptions, &$arrField, &$arrErrors=null ) {
			...
			switch ( $arrField['type'] ) {
				...
				case 'week':	// attributes: size
					...
					$strOutput .= "<input id='{$strTagID}' class='{$arrField['class']}' name='{$strFieldName}' size='{$arrField['size']}' type='{$arrField['type']}' value='{$strValue}' />"; 			
					break;
				case 'number':	// HTML5, attributes: min, max, step,
				case 'range':						
					$strOutput .= "<input id='{$strTagID}' class='{$arrField['class']}' name='{$strFieldName}' min='{$arrField['min']}' max='{$arrField['max']}' step='{$arrField['step']}' type='{$arrField['type']}' value='{$strValue}' />";				
					break;
				case 'textarea':	// attributes: rows, cols
					...
					$strOutput .= "<textarea id='{$strTagID}' class='{$arrField['class']}' name='{$strFieldName}' rows='{$arrField['rows']}' cols='{$arrField['cols']}'>{$strValue}</textarea>";
					break;	
				case 'radio':
					$strOutput .= "<div id='{$strTagID}'>";
					foreach ( $arrField['label'] as $strKey => $strLabel ) {
						$strChecked = ( $strValue == $strKey ) ? 'Checked' : '';
						$strOutput .= "<input id='{$strTagID}_{$strKey}' class='{$arrField['class']}' type='radio' name='{$strFieldName}' value='{$strKey}' {$strChecked}>&nbsp;&nbsp;{$strLabel}<br />";
					}
					$strOutput .= "</div>";
					break;
				case 'checkbox':	// support multiple creation with array of label
					...
					if ( is_array( $arrField['label'] ) ) {
						...
						foreach ( $arrField['label'] as $strKey => $strLabel ) {	
							...
							$strOutput .= "<input id='{$strTagID}_{$strKey}' class='{$arrField['class']}' type='checkbox' name='{$strFieldName}[{$strKey}]' value='1' {$strChecked}/>&nbsp;&nbsp;{$strLabel}<br />";
						}
						$strOutput .= "</div>";
						break;
					}
					...
					$strOutput .= "<input id='{$strTagID}' class='{$arrField['class']}' type='checkbox' name='{$strFieldName}' value='1' {$strChecked}/>&nbsp;&nbsp;{$arrField['label']}<br />";
					break;
				case 'select':
					if ( !is_array( $arrField['label'] ) ) break;	// the label key must be an array for the select type.
					$strOutput .= "<select id='{$strTagID}' class='{$arrField['class']}' name='{$strFieldName}'>";
					foreach ( $arrField['label'] as $strKey => $strLabel ) {
				...
				case 'hidden':	// support multiple creation with array of label
					...
					$strOutput .= "<input id='{$strTagID}' class='{$arrField['class']}' name='{$strFieldName}' type='hidden' value='{$arrField['label']}' />";
					break;					
				case 'file':	// support multiple creation with array of label
					...
					$strOutput .= "<input id='{$strTagID}' class='{$arrField['class']}' type='file' name='{$strFieldName}' />";
					break;
				case 'submit':	// support multiple creation with array of label
					...
					if ( is_array( $arrField['label'] ) ) {
						...
						foreach( $arrField['label'] as $strArrayKey => $strArrayValue ) {
							...
							$strOutput .= "<input id='{$strTagID}_{$strArrayKey}' class='{$strClass}' name='{$strFieldName}[{$strArrayKey}]' type='submit' value='{$strLabel}' />&nbsp;&nbsp;";
						}
						$strOutput .= "</div>";
						break;
					}
					$strLabel = ( $arrField['label'] ) ? $arrField['label'] : __( 'Submit' );
					$strOutput .= "<input id='{$strTagID}' class='{$strClass}' name='{$strFieldName}' type='submit' value='{$strLabel}' />";
					break;
				default:	
					// for anything else, do nothing.
					return;	// do not continue on the flow of execution outside the switch 
		...
		protected function RenderPage( $strPageSlug, $strTabSlug=null ) {
			...
			<div class="wrap">
				...
				?>
				<div class="admin-page-framework-container" style="margin-top:20px;">
